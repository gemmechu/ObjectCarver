...
class Runner:
    def __init__(self, conf_path, scene_name = '', mode='train', model_type='', is_continue=False, checkpoint_id = -1, exp_name= ""):
    ...
    def train_neus(self):
        ...
        for iter_i in tqdm(range(res_step)):  
            ...
            bound = 0.9
            num_points = 10000
            random_points = torch.rand(num_points, 3).cuda().float() * (2*bound) - bound
            batch_size = 500
            minimum_points = torch.zeros(len(self.dataset.objects),num_points)
            ...
            for idx, obj_key in enumerate(self.dataset.objects):
                ...
                if self.overlap_weight!=0:
                    for i in range(0,num_points, batch_size):
                        x = random_points[i:i+batch_size]
                        y = self.sdf_networks[obj_key](x)[:,:1]
                        
                        minimum_points[idx][i:i+batch_size] = (torch.min(y, torch.zeros_like(y))).squeeze(1)
            ...
            if self.overlap_weight!=0:
                ...
                minimum_points[min_indices_axis_0, torch.arange(num_points)] = 0
            ...
            wandb.log({"overlap_loss": overlap_loss}, step=self.iter_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='', help='Scene or scan name')
    args = parser.parse_args()

    torch.cuda.set_device(args.gpu)
    runner = Runner(args.conf, args.scene_name, args.mode, args.model_type, args.is_continue, args.checkpoint_id, args.exp_name)
