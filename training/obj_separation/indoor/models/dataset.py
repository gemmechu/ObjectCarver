...
from utils.utils_image import read_images, write_image, write_images,read_images_modified
...
def convert_image_to_binary(image, target_value):
    ...
    # binary_image = np.where(image == target_value, 255, 0).astype(np.uint8)
...
class Dataset:
    ...
    '''
    def __init__(self, conf,instance_seg):
        ...
        self.instance_segmentation = json.load(open(os.path.join(self.data_dir, instance_seg)))
        ...
        for idx in range(len(self.masks_lis)):
            binary_mask_all = []
            self.masks[idx] = {}
            # gray_mask = np.where(cv.imread(self.masks_lis[idx], 0) > 0, 255, 0).astype(np.uint8)
            # binary_mask_all = gray_mask.reshape((gray_mask.shape[0], gray_mask.shape[1], 1))
            ...
            # cv.imwrite("images_np.png",self.images_np[idx]*255)
            # cv.imwrite("binary_mask_all.png",binary_mask_all)
            for obj_key in self.objects:
                ...
                mask_amodal = cv.imread(os.path.join(self.data_dir, "amodal",obj_key, img_name))[...,0]/255
        ...
        #     # self.masks_np = np.stack([convert_image_to_binary(cv.imread(im_name), target_value = self.instance_segmentation["1"]) for im_name in masks_lis])/ 256.0
        ...
        for scale_mat, world_mat in zip(self.scale_mats_np, self.world_mats_np):
            ...
            if conf["camera_crop"]:
                ...
                W, H = 1200, 680
                min_H_W = min([W,H])
                offset_x = (W - min_H_W) * 0.5
                offset_y = (H - min_H_W) * 0.5
                intrinsics[0, 2] -= offset_x
                intrinsics[1, 2] -= offset_y
                resize_factor = target_size / min_H_W
        ...
        if self.use_normal:
            ...
            normals_npz, stems_normal = read_images_modified(f'{self.data_dir}/pred_normal', target_img_size=(w_img, h_img), img_ext='.npy')
    ...
    def initialize_patchmatch(self):
        ...
        self.samples_accum = torch.zeros_like(self.images, dtype=torch.int32).cuda()
    ...
    def random_get_rays_at(self, img_idx, batch_size, pose = None):
        ...
        return torch.cat([rays_o.cpu(), rays_v.cpu(), color], dim=-1).cuda(), mask_list.cuda(), pixels_x, pixels_y, normal_sample, planes_sample, subplanes_sample  # batch_size, 10
