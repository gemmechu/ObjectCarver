...
class NeuSLoss(nn.Module):
    ...
    def forward(self, input_model, render_out, sdf_network_fine, patchmatch_out = None, idx=None):
        true_rgb = input_model['true_rgb']

        mask, rays_o, rays_d, near, far, mask_dcare = input_model['mask_list'][idx], input_model['rays_o'], input_model['rays_d'],  \
                                                    input_model['near'], input_model['far'], input_model["mask_dcare_list"][idx]
        ...
        if True:
            ...
            mask_clone[mask_dcare.squeeze() == 1] = weight_sum_clone[mask_dcare.squeeze() == 1]
            background_loss = F.binary_cross_entropy(weight_sum, mask_clone) #hmm can't i just multiply by the other loss
            ########## new code ends #################
        ...
        if self.normal_weight > 0 and normals_target is not None:
            ...
            normal_certain_weight = normal_certain_weight*(mask==1)
        ...
        # loss = background_loss * self.mask_weight 
