...
class Runner:
    def __init__(self, conf_path, scene_name = '', mode='train', model_type='', is_continue=False, checkpoint_id = -1, exp_name= "", instance_seg = "instance_segmentation.json"):
        ...
        self.cpkt_start = self.conf['general.cpkt_start']
        self.npz_file = self.conf['general.npz_file']
    ...
    def load_pretrained_model(self):
        # Load checkpoint
        if self.cpkt_start != "":
            self.load_checkpoint(self.cpkt_start)
            return
    ...
    def parse_parameters(self):
        ...
        self.base_exp_dir = os.path.join(self.conf['general.exp_dir'], self.dataset_type,"obj_separation", self.scan_name, str(self.exp_name))
        print("self.base_exp_dir: ", self.base_exp_dir)
        ...
        self.overlap_batch_size = self.conf.get_int('train.overlap_batch_size', default=500)
        ...
        self.overlap_weight = self.conf.get_float('model.loss.overlap_weight')
    def build_dataset(self):
        ...
        self.dataset = Dataset(self.conf['dataset'], self.instance_seg)
    ...
    def build_model(self):
        ...
        if self.model_type == 'neus':
            ...
            for obj_key in self.dataset.objects:
                ...
                renderer =  NeuSRenderer(self.nerf_outside,sdf_network,self.variance_network_fine,self.color_network_fine,**self.conf['model.neus_renderer'])
    ...
    def get_model_input(self, image_perm, iter_i):
        ...
        data, masks, pixels_x, pixels_y,  normal_sample, planes_sample, subplanes_sample = self.dataset.random_get_rays_at(idx_img, self.batch_size)
        ...
        for idx, obj_key in enumerate(self.dataset.objects):
            ...
            mask_dcare = (masks[idx][1] > 0.5).float()
            mask_sum = mask.sum() + 1e-5
            mask_list.append(mask)
            mask_sum_list.append(mask_sum) 
            mask_dcare_list.append(mask_dcare)
        ...
        input_model.update({
            ...
            'pixels_uv': pixels_uv,
            'pixels_vu': pixels_vu,
            'mask_dcare_list': mask_dcare_list,
        })
        return input_model, logs_input

    def train_neus(self):
        conf_dict = read_config_from_file(self.conf_path)
        # wandb.init(project="NeuRIS Replica", name=self.exp_name, config=conf_dict)
        ...
        for iter_i in tqdm(range(res_step)):  
            ...
            for idx, obj_key in enumerate(self.dataset.objects):
                render_out, logs_render = self.renderers[obj_key].render(input_model['rays_o'], input_model['rays_d'], 
                ...
                loss_idx, logs_loss, mask_keep_gt_normal = self.loss_neus(input_model, render_out, self.sdf_networks[obj_key], patchmatch_out, idx=idx)
                loss += loss_idx
            ...
            # wandb.log({"Loss/overlap_loss": overlap_loss})
            ...
            # wandb.log(logs_summary)
            # if self.iter_step %100 ==0:
                
            #     # image = PIL.Image.fromarray(image_path)
            #     image = render_out["color_fine"]
                
            #     wandb.log({"Images/color_fine": wandb.Image(image)})
    ...
    def update_iter_step(self):
        ...
        for obj_key in self.dataset.objects:
                self.sdf_networks[obj_key].iter_step = self.iter_step
                self.sdf_networks[obj_key].end_iter = self.end_iter
    ...
    def load_checkpoint(self, checkpoint_name):
        if self.cpkt_start != "":
            checkpoint = torch.load(self.cpkt_start, map_location=self.device)
        else:
    ...
    def validate(self, input_model, loss_out, render_out):
        mask_list, rays_o, rays_d, near, far = input_model['mask_list'], input_model['rays_o'], input_model['rays_d'],  \
                                                    input_model['near'], input_model['far']
        ...
        if self.iter_step % self.val_mesh_freq == 0 or self.iter_step == 1:
            for idx, obj_key in enumerate(self.dataset.objects):
                self.validate_mesh(renderer= self.renderers[obj_key], obj_key=obj_key)
        
        if self.iter_step % self.val_image_freq == 0:
            for idx, obj_key in enumerate(self.dataset.objects):
                self.validate_image(renderer= self.renderers[obj_key], save_normalmap_npz=self.save_normamap_npz,obj_key=obj_key)
    ...
    def validate_image(self, idx=-1, resolution_level=-1, 
                                ...
                                save_image_render = False, renderer=None, obj_key=None):
        idx = 70
        # validate image
        ...
        for rays_o_batch, rays_d_batch, idx_pixels_vu_batch in zip(rays_o, rays_d, idx_pixels_vu):
            ...
            render_out, _ = renderer.render(rays_o_batch, rays_d_batch, near, far, alpha_inter_ratio=self.get_alpha_inter_ratio(), background_rgb=background_rgb)
        ...
        ImageUtils.write_image_lis(f'{dir_images}/{obj_key}_{self.iter_step:08d}_reso{resolution_level}_{idx:08d}.png',
    ...
    def validate_mesh(self, world_space=False, resolution=128, threshold=0.0, renderer=None,obj_key=None):
        ...
        vertices, triangles, sdf = renderer.extract_geometry(bound_min, bound_max, resolution=resolution, threshold=threshold)
        if resolution>128:
        ...
        path_mesh = os.path.join(out_dir, f'{obj_key}_{self.iter_step:0>8d}_reso{resolution}_{self.scan_name}.ply')
        ...
        if world_space:
            ...
            path_mesh = os.path.join(out_dir, f'{obj_key}_{self.iter_step:0>8d}_reso{resolution}_{self.scan_name}_world.ply')
            path_mesh_gt = IOUtils.find_target_file(self.dataset.data_dir, '_vh_clean_2.ply')

        mesh = trimesh.Trimesh(vertices, triangles)
        #====color mesh begins=======#
        rgb_color = self.dataset.instance_segmentation[obj_key]
        color_float = [c / 255.0 for c in rgb_color]
        mesh.visual.vertex_colors = [color_float] * len(mesh.vertices)
        #====color mesh ends=======#
    ...
    def initialize_accumulated_results(self, mode = 'MODEL', iter_step_npz = None, resolution_level = 2):
        ...
        if mode == 'model':
            ...
            self.dataset.confidence_accum = np.load(self.npz_file)['arr_0']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--instance_seg', type=str, default='instance_segmentation.json', help='instance segmentation name')
    args = parser.parse_args()

    torch.cuda.set_device(args.gpu)
    runner = Runner(args.conf, args.scene_name, args.mode, args.model_type, args.is_continue, args.checkpoint_id, args.exp_name, args.instance_seg)
    ...
    if args.mode == 'train':
        ...
        if runner.model_type == 'neus':
            for obj_key in runner.dataset.objects:
                runner.validate_mesh(world_space=True, resolution=args.mc_reso, threshold=args.threshold, renderer=runner.renderers[obj_key], obj_key=obj_key)
            # t1= datetime.now()
